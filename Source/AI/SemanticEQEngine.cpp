...
SemanticEQEngine::parseNaturalLanguage(const juce::String& input) const
{
    ...
    juce::String lowInput = input.toLowerCase();
    
    // Check for "more" or "less" modifiers
    bool hasMore = lowInput.contains("more") || lowInput.contains("piu") || 
                   lowInput.contains("più") || lowInput.contains("add");
    bool hasLess = lowInput.contains("less") || lowInput.contains("meno") || 
                   lowInput.contains("reduce") || lowInput.contains("remove");
    
    float modifier = hasLess ? -1.0f : 1.0f;
        ...
        for (const auto& mapping : langPair.second)
        {
            for (const auto& keyword : mapping.keywords)
            {
                if (lowInput.contains(keyword))
                {
                    ...
                    if (mapping.isNegative) amount = -amount;
                    
                    // Check if this quality is already in results
                    bool found = false;
                    for (auto& existing : results)
                    {
                        if (existing.first == mapping.quality)
                        {
                            existing.second = amount;  // Update
                            found = true;
                            break;
                        }
                    }
                    ...
                    if (!found)
                        results.push_back({ mapping.quality, amount });
                }
            }
        }
    }
