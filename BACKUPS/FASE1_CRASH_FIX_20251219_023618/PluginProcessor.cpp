      ...
      apvts(*this, nullptr, "Parameters", createParameters())
{
    ...
    irBuilderShouldExit.store(false, std::memory_order_relaxed);
...
void AIEqualizerAudioProcessor::irBuilderThreadFunc()
{
    ...
    while (!irBuilderShouldExit.load(std::memory_order_relaxed))
    {
        irBuildEvent.wait(-1);
        
        // Check exit flag after wake
        if (irBuilderShouldExit.load(std::memory_order_relaxed))
            break;
...
AIEqualizerAudioProcessor::~AIEqualizerAudioProcessor()
{
    ...
    // Signal IR builder thread to stop and wait for it to finish
    irBuilderShouldExit.store(true, std::memory_order_release);
    irBuildEvent.signal();
...
void AIEqualizerAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    {
        msBuffer.setSize(2, samplesPerBlock * 4, false, false, true);
        msBuffer.clear();
    }
    
    // FIX 5: Pre-allocate M/S processing buffers with 4x headroom
    midProcessBuffer.setSize(1, samplesPerBlock * 4, false, false, true);
    sideProcessBuffer.setSize(1, samplesPerBlock * 4, false, false, true);
    ...
    naturalOversampledBuffer.setSize(getTotalNumInputChannels(), hqBlockSize * 4, false, false, true);
    ...
    crossfadeBuffer.setSize(getTotalNumInputChannels(), samplesPerBlock * 4);
    ...
    // FIX 8: Calculate latency after prepare
                                              ...
                                              juce::MidiBuffer& /*midiMessages*/)
{
    ...
    if (qualityMode != qualityModeCached)
    {
        ...
        dynamicEQProcessor.setLookahead(lookaheadMs);
        dynamicEQProcessor.updateLookaheadBuffer(currentSampleRate.load(), currentBlockSize, totalNumInputChannels);
    }
    ...
    const bool autoGainEnabledLocal = apvts.getRawParameterValue("autoGain")->load() > 0.5f;
    ...
    if (mode == PhaseMode::LinearPhase)
    {
        ...
        // Use single convolver - JUCE's Convolution internally crossfades when IR is updated
        // No external crossfade needed; JUCE handles smooth transitions internally
        auto* lp = linearPhaseProcessors[static_cast<size_t>(currentIR)].get();
        if (lp)
        {
            juce::dsp::AudioBlock<float> block(buffer);
...
void AIEqualizerAudioProcessor::updateLinearPhaseIRIfNeeded()
{
    ...
    {
        // Switch to the new IR - JUCE's Convolution handles internal crossfading
...
//==============================================================================
// FIX 8: Calculate latency outside of processBlock
// FIX: Use WeakReference to prevent dangling pointer crash if plugin is destroyed during async call
void AIEqualizerAudioProcessor::updateReportedLatency()
{
    // FIX: setLatencySamples must be called on Message Thread to avoid DAW instability
    // Use WeakReference to safely handle the case where plugin is destroyed before callback executes
    juce::WeakReference<AIEqualizerAudioProcessor> weakThis(this);
    
    juce::MessageManager::callAsync([weakThis]()
    {
        // CRITICAL: Check if plugin still exists before accessing
        auto* self = weakThis.get();
        if (self == nullptr)
            return;  // Plugin was destroyed, abort safely
        
        int totalLatency = 0;
        
        // Oversampling latency (only for Natural Phase mode)
        const auto mode = self->currentPhaseMode.load();
        
        if (mode == PhaseMode::NaturalPhase)
        {
            int osFactor = self->oversamplingFactor.load();
            if (osFactor == 1 && self->oversampler2x)
                totalLatency += static_cast<int>(self->oversampler2x->getLatencyInSamples());
            else if (osFactor == 2 && self->oversampler4x)
                totalLatency += static_cast<int>(self->oversampler4x->getLatencyInSamples());
            else if (self->naturalOversampler)
                totalLatency += static_cast<int>(self->naturalOversampler->getLatencyInSamples());
        }
        
        // Linear phase convolution latency: SAMPLE-ACCURATE using IR size
        // The latency of a linear-phase FIR is exactly (irSize - 1) / 2 samples
        if (mode == PhaseMode::LinearPhase)
        {
            totalLatency += static_cast<int>(LinearPhaseProcessor::irSize / 2);
        }
        
        if (totalLatency != self->lastReportedLatency)
        {
            self->setLatencySamples(totalLatency);
            self->lastReportedLatency = totalLatency;
        }
    });
}
