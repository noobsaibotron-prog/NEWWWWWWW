...
public:
    ...
    static constexpr float tiltDbPerOct = 4.5f;
...
private:
    ...
    {
        if (auto* p = processor.getAPVTS().getRawParameterValue("analyzerTilt"))
            return p->load() > 0.5f;
        return true;
    }
    ...
    float applyTilt(float db, float freq) const
    {
        ...
        return static_cast<float>(db + octaves * tiltDbPerOct);
    }
    ...
    void showContextMenu(juce::Point<int> pos)
    {
        ...
        menu.addSeparator();
        menu.addItem(30, "Tilt Compensation (4.5 dB/oct)", true, tilt);
        menu.addItem(31, "Piano Roll Overlay", true, pianoRoll);

        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea({pos, {1, 1}}),
            [this, showPre, showPost, showDelta, tilt, pianoRoll](int result)
            {
                switch (result)
                {
                    ...
                    case 30: setBoolParameter("analyzerTilt", !tilt); break;
                    case 31: setBoolParameter("pianoRollOverlay", !pianoRoll); break;
                    default: break;
                }
            });
    }
    ...
    void updateSmoothedSpectrum()
    {
        ...
        if (smoothedSpectrum.size() != w) smoothedSpectrum.resize(w, spectrumMinDb);
