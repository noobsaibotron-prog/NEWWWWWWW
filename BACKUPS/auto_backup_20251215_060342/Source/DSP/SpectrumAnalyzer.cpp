...
{
    // CRITICAL FIX: Use std::lock_guard for prepare (called from non-audio thread, can wait)
...
{
    // CRITICAL FIX: Use std::lock_guard for reset (called from non-audio thread, can wait)
    std::lock_guard<std::mutex> lock(dataLock);
    std::fill(fifoBuffer.begin(), fifoBuffer.end(), 0.0f);
    std::fill(fftData.begin(), fftData.end(), 0.0f);
    std::fill(rawSpectrum.begin(), rawSpectrum.end(), minDecibels);
...
void SpectrumAnalyzer::process(const juce::AudioBuffer<float>& buffer)
{
    ...
    // CRITICAL FIX: Use try_lock with timeout in audio thread to prevent latency spikes
    // If lock cannot be acquired quickly, skip this update to avoid blocking audio processing
    std::unique_lock<std::mutex> lock(dataLock, std::try_to_lock);
    if (!lock.owns_lock())
    {
        // Try with timeout (100 microseconds max wait)
        lock = std::unique_lock<std::mutex>(dataLock, std::defer_lock);
        if (!lock.try_lock_for(lockTimeoutUs))
        {
            // CRITICAL FIX: Skip update if lock contention - prevents latency spikes
            // This is better than blocking audio processing
            return;
        }
    }
...
void SpectrumAnalyzer::setFFTResolution(Resolution res)
{
    ...
    // CRITICAL FIX: Use std::lock_guard for setFFTResolution (called from non-audio thread, can wait)
    std::lock_guard<std::mutex> lock(dataLock);
    resolution = res;
    rebuildFFT(newOrder);
}
