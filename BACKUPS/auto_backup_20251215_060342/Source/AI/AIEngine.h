...
private:
    ...
    // CRITICAL FIX: Use shared_mutex to allow concurrent reads from audio thread
    // while GUI thread can modify (rejectCorrection). Read lock for getPendingCorrections(),
    // write lock for rejectCorrection() and other modifications.
    mutable std::shared_mutex correctionsMutex;
