                                              ...
                                              juce::MidiBuffer& /*midiMessages*/)
{
    ...
    if (mode == PhaseMode::LinearPhase)
    {
        ...
        // If IR not yet loaded, fall back to zero-latency EQ to avoid silence
...
void AIEqualizerAudioProcessor::saveCurrentStateToSlot(ABState slot)
{
    ...
    // SAFETY: Ensure slot is properly sized
    targetSlot->bands.resize(static_cast<size_t>(maxBands));
...
void AIEqualizerAudioProcessor::loadStateFromSlot(ABState slot)
{
    ...
    // SAFETY: Ensure slot is properly initialized
    // If slot was never saved, initialize it with current state first
    if (sourceSlot->bands.empty() || sourceSlot->bands.size() < static_cast<size_t>(maxBands))
    {
        // Slot not initialized - save current state to it first
    ...
    // SAFETY: Clamp band count to valid range
    const int bandsToLoad = juce::jlimit(0, maxBands, static_cast<int>(sourceSlot->bands.size()));
    
    // Load band states with bounds checking
    for (int i = 0; i < bandsToLoad && i < maxBands; ++i)
    {
