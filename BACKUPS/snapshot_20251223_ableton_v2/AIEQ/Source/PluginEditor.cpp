...
void AIEqualizerAudioProcessorEditor::createControlPanel()
{
    ...
    captureAnalyzeBtn.onClick = [this]() {
        ...
        captureStatusLabel.setText(ok ? "Retro: last " + juce::String(sec, 1) + "s analyzed"
                                       : "No audio captured",
    ...
    stopCaptureBtn.onClick = [this]() {
        ...
        if (ok)
        {
            ...
            captureStatusLabel.setText("Live: " + juce::String(secs, 1) + "s analyzed",
                                       juce::dontSendNotification);
            captureStatusLabel.setColour(juce::Label::textColourId, juce::Colours::limegreen);
        }
        else
        {
            captureStatusLabel.setText("No audio captured", juce::dontSendNotification);
            captureStatusLabel.setColour(juce::Label::textColourId, juce::Colours::orange);
        }
...
void AIEqualizerAudioProcessorEditor::resized()
{
    ...
    const int togglesW = 260;
    const int oversamplingW = 110;
    const int numBandsW = 90;
    const int logoW = 100;
    ...
    // Capture controls (expanded area) - limit width to leave room for band selector
    const int reserveTail = knobW * 2 + oversamplingW + togglesW + numBandsW + logoW + 40;
    int captureWidth = cpanel.getWidth() - reserveTail;
    if (captureWidth > 0)
        captureWidth = juce::jlimit(140, 260, captureWidth);
    else
        captureWidth = 0;
    captureWidth = std::min(captureWidth, cpanel.getWidth());
    auto captureArea = cpanel.removeFromLeft(captureWidth);
    auto capLabelArea = captureArea.removeFromTop(14);
    captureLenLabel.setBounds(capLabelArea);
    auto capSliderArea = captureArea.removeFromTop(22);
    ...
    captureAnalyzeBtn.setBounds(btnRow1.removeFromLeft(170).reduced(2));
    autoCaptureBtn.setBounds(btnRow1.reduced(2));
    
    // Button row: Manual START/STOP
    auto btnRow2 = captureArea.removeFromTop(26);
    startCaptureBtn.setBounds(btnRow2.removeFromLeft(130).reduced(2));
    stopCaptureBtn.setBounds(btnRow2.reduced(2));
    
    auto statusRow = captureArea.removeFromTop(18);
    captureStatusLabel.setBounds(statusRow);
    
    if (captureWaveform)
        captureWaveform->setBounds(captureArea.reduced(0, 2));
    ...
    cpanel.removeFromLeft(6);
    auto osArea = cpanel.removeFromLeft(oversamplingW);
    oversamplingLabel.setBounds(osArea.removeFromTop(14));
    oversamplingCombo.setBounds(osArea.removeFromTop(26).reduced(2));

    cpanel.removeFromLeft(10);

    // Band toggles
    auto toggleArea = cpanel.removeFromLeft(togglesW);
    auto toggleRow = toggleArea.removeFromTop(30);
    for (int i = 0; i < 4; ++i)
        bandToggles[i].setBounds(toggleRow.removeFromLeft(60).reduced(2));
    toggleRow = toggleArea.removeFromTop(30);
    for (int i = 4; i < 8; ++i)
        bandToggles[i].setBounds(toggleRow.removeFromLeft(60).reduced(2));
    ...
    numBandsLabel.setBounds(numBandsArea.removeFromTop(16));
    numBandsCombo.setBounds(numBandsArea.removeFromTop(30).reduced(2));
...
void AIEqualizerAudioProcessorEditor::timerCallback()
{
    ...
    // Update capture waveform preview
    {
        ...
        if (captureWaveform)
            captureWaveform->setData(recording ? preview : lastCapture,
