...
void AIEqualizerAudioProcessorEditor::createControlPanel()
{
    ...
    // Capture & analyze button (records last audio and runs AI analysis)
    captureAnalyzeBtn.setTooltip("Cattura audio e avvia analisi AI");
...
void AIEqualizerAudioProcessorEditor::resized()
{
    ...
    // Capture/Analyze button and length slider
    auto captureArea = cpanel.removeFromLeft(200);
    ...
    captureAnalyzeBtn.setBounds(captureArea.removeFromTop(26).reduced(2));
    captureStatusLabel.setBounds(captureArea);
    
    // Selected band panel (center-right of control panel)
    cpanel.removeFromLeft(15);
    auto bandDetailArea = cpanel.removeFromLeft(360);
