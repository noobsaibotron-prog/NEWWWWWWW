...
void ParametricEQProcessor::process(juce::AudioBuffer<float>& buffer)
{
    ...
    // Copy band state once under lock, then process lock-free
    std::array<bool, getMaxBands()> localEnabled{};
    std::array<bool, getMaxBands()> localSolo{};
    std::array<bool, getMaxBands()> localVintage{};
    std::array<FilterType, getMaxBands()> localType{};
    int localNumBands = 0;
    bool hasSolo = false;
    
    {
        ...
        localNumBands = std::min(getMaxBands(), static_cast<int>(bands.size()));
        
        for (int i = 0; i < localNumBands; ++i)
        {
            auto& band = bands[static_cast<size_t>(i)];
            localEnabled[i] = band.enabled;
            localSolo[i] = band.solo;
            localVintage[i] = band.vintageMode;
            localType[i] = band.type;
    ...
    // Process each band
    for (int idx = 0; idx < localNumBands; ++idx)
    {
        auto& band = bands[static_cast<size_t>(idx)];
        
        // Skip if not enabled, or if other bands are solo'd and this one isn't
        if (!localEnabled[idx])
            continue;
        if (hasSolo && !localSolo[idx])
            continue;
        
        const bool vintage = localVintage[idx];
        const FilterType type = localType[idx];
        ...
        if (bufferChannels > 0)
        {
            ...
            for (int i = 0; i < numSamples; ++i)
            {
                ...
                if (vintage && (type == VintageLowShelf || type == VintageHighShelf))
                {
        ...
        if (bufferChannels > 1)
        {
            ...
            for (int i = 0; i < numSamples; ++i)
            {
                float sample = band.filterR.processSample(channelData[i]);
    ...
    const int localNumChannels = buffer.getNumChannels();
    // numSamples already captured
    
    for (int ch = 0; ch < localNumChannels; ++ch)
    {
