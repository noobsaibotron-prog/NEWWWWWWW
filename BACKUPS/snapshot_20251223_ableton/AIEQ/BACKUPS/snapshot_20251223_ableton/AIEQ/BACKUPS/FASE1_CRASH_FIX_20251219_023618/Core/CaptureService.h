...
public:
    ...
    void prepare(double sampleRate, int numChannels, int maxBlockSize)
    {
        ...
        // Ring buffer for retroactive capture (last N seconds)
        const int ringCapacity = static_cast<int>(sampleRate * kMaxCaptureSeconds);
        ringBuffer.prepare(numChannels, ringCapacity);
        
        // Manual capture buffer (pre-allocated, stereo interleaved)
        // Size: max seconds * sample rate * 2 channels
        const size_t manualCapacity = static_cast<size_t>(sampleRate * kMaxCaptureSeconds * 2);
    ...
    void stopManualCapture()
    {
        ...
        const size_t numAudioSamples = totalSamples / numChannels;
        ...
        for (size_t i = 0; i < numAudioSamples; ++i)
        {
            ...
            {
                const size_t idx = i * numChannels + ch;
                if (idx < totalSamples && idx < manualCaptureBuffer.size())
                {
                    sum += manualCaptureBuffer[idx];
                }
            }
            capturedAudioMono[i] = sum / static_cast<float>(numChannels);
        }
    }
    ...
    void getManualCapturePreview(std::vector<float>& outMono, size_t maxSamples) const
    {
        ...
        for (size_t i = 0; i < numAudioSamples; i += step)
        {
            ...
            outMono.push_back(sum / static_cast<float>(numChannels));
        }
    }
...
private:
    ...
    {
        const int numSamples = buffer.getNumSamples();
        const int channelsToCopy = std::min(buffer.getNumChannels(), 2);
