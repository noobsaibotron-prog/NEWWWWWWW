...
void DynamicEQProcessor::prepare(double sampleRate, int samplesPerBlock, int channels)
{
    ...
    // FIX: preallocate dry buffer with headroom (4x block) to keep process lock-free
    ...
    for (int i = 0; i < maxBands; ++i)
        updateBandCoefficients(i, bandParams[i]);
}
...
{
    // Resize lookahead buffer based on current lookaheadMs
    lookaheadSamples = static_cast<int>((lookaheadMs / 1000.0f) * sampleRate);
    if (lookaheadSamples > 0)
    {
        lookaheadBuffer.setSize(channels, lookaheadSamples + samplesPerBlock, false, false, true);
        lookaheadBuffer.clear();
    }
    else
    {
...
void DynamicEQProcessor::process(juce::AudioBuffer<float>& buffer)
{
    ...
    // Copy parameters once before processing (avoid per-band locks)
    std::array<DynamicBandParams, maxBands> localParams;
    {
        std::lock_guard<std::mutex> lock(paramsMutex);
        localParams = bandParams;
    }

    // Prepare dry buffer for global mix without allocations
    if (globalMix < 0.999f)
    {
    ...
    {
        const auto& params = localParams[bandIdx];
        ...
        {
            updateBandCoefficients(bandIdx, params);
...
//==============================================================================
void DynamicEQProcessor::updateBandCoefficients(int bandIndex, const DynamicBandParams& params)
{
    if (bandIndex < 0 || bandIndex >= maxBands)
        return;
    ...
    int filterType, float freq, float gain, float q) const
{
    ...
    switch (filterType)
    {
        ...
        case 5: // Notch
            return juce::dsp::IIR::Coefficients<float>::makeNotch(currentSampleRate, freq, q);
        case 6: // BandPass
