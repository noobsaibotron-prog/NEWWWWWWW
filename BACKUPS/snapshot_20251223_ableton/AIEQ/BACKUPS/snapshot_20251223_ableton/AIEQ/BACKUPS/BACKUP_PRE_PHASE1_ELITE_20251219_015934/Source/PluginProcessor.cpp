...
void AIEqualizerAudioProcessor::updateReportedLatency()
{
    ...
    // Use callAsync to ensure thread-safety when called from parameterChanged (which may be audio thread)
    juce::MessageManager::callAsync([this]()
    {
        ...
        if (mode == PhaseMode::NaturalPhase)
        {
            ...
            else if (naturalOversampler)
                totalLatency += static_cast<int>(naturalOversampler->getLatencyInSamples());
        }
