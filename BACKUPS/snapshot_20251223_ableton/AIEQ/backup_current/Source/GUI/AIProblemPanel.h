...
private:
    ...
    {
        // Use filtered and prioritized corrections (with severity/confidence thresholds)
        auto filtered = processor.getAIEngine().getFilteredAndPrioritizedCorrections(0.25f, 0.55f);
        // Merge nearby corrections to avoid showing duplicates
        problems = processor.getAIEngine().mergeNearbyCorrections(filtered);
